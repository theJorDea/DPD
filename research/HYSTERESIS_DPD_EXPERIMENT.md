...
- дополнительно 9 конфигураций с residual только в одной из трёх ветвей;
...
### Sparse one-branch ablation

Для deadband `alpha=0.5` residual добавлялся только в одной ветви:

| Активная ветвь | Лучший проверенный residual ridge | Изменение к baseline |
|---:|---:|---:|
| `(0,0)` | `1.0` | `-0.00061 dB` |
| `(1,1)` | `1.0` | `-0.00255 dB` |
| `(2,2)` | `1.0` | `-0.00199 dB` |

При меньшем residual ridge ухудшение возрастает до `0.274 dB`. Следовательно,
отрицательный результат не объясняется только удвоением числа свободных
параметров: даже одна residual-группа не дала выигрыша.
