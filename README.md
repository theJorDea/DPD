...
## 11. Каноническая структура проекта

Рабочие материалы разделены по назначению:

```text
baseline/                  библиотека моделей и расчёта сложности
  spline_memory_dpd.py     зафиксированный Spline Memory DPD baseline
  fixed_point_*.py         отдельные fixed-point реализации
experiments/               запускаемые проверки и конфигурации
  configs/                 неизменяемые параметры запусков
  results/                 машиночитаемые результаты и манифесты
research/                  аудиты, литературный обзор и решения по отбору
tests/                     unit- и интеграционные тесты
docs/notebooks/            три notebook-файла и подробный HTML
  Spline_memory_DPD.ipynb
  Spline_memory_DPD_detailed.ipynb
  Spline_memory_DPD_research.ipynb
```

`Spline_memory_DPD.ipynb` — прежняя воспроизводимая версия. Детальная версия
используется для новых диагностик, но не подменяет baseline без отдельного
подтверждённого эксперимента. `Spline_memory_DPD_research.ipynb` оформляет
тот же BlackBox-эксперимент как исполняемую исследовательскую статью:
мотивация, источники, формулы, ablation, результаты, сложность и границы
доказательности. Отклонённые варианты не входят в рабочий datapath и не
должны смешиваться с baseline-результатами.

## 12. Воспроизведение одной командой
...
Последняя полная проверка в окружении NumPy: `393` теста пройдено, `16` ожидаемо пропущены из-за отсутствия
...
## 13. Следующий решающий эксперимент
