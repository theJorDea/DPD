r"""Causal proper-complex FIR residual correction for a frozen PA model.
...
.. math::
   ...
   \qquad
   \hat y_{FIR}[n] = \hat y_{base}[n] + \Delta\hat y[n].

All delays are non-negative.  Independent records use zero history, while
continuous streaming carries an explicit raw complex input state.  The model
is phase equivariant and calibration uses column-scaled complex augmented
least squares without an intercept or normal equations.
"""
...
def _normalize_delays(delays: Iterable[int]) -> tuple[int, ...]:
    values = tuple(delays)
    if not values:
        raise ValueError("complex FIR delays must not be empty")
    if any(
        isinstance(delay, bool) or not isinstance(delay, Integral)
        for delay in values
    ):
        raise TypeError("complex FIR delays must be integers")
    result = tuple(int(delay) for delay in values)
    if any(delay < 0 for delay in result):
        raise ValueError("complex FIR delays must be causal and non-negative")
    if len(set(result)) != len(result):
        raise ValueError("complex FIR delays must be unique")
    return result
...
def complex_fir_design_matrix(
    signal: np.ndarray,
    delays: Iterable[int],
) -> np.ndarray:
    """Return columns ``x[n-d]`` with zero pre-record context."""

    samples = _complex_vector(signal, name="signal")
    delay_tuple = _normalize_delays(delays)
    return np.column_stack(
        [_causal_delay(samples, delay) for delay in delay_tuple]
    )


def complex_fir_segmented_design_matrix(
    signal: np.ndarray,
    delays: Iterable[int],
    *,
    segment_length: int,
...
class ComplexFIRStreamingState:
    ...
    def __post_init__(self) -> None:
        ...
        if history.ndim != 1:
            raise ValueError("complex FIR streaming history must be 1-D")
        history = np.asarray(history, dtype=np.complex128)
        if not np.all(np.isfinite(history)):
            raise ValueError("complex FIR streaming history must be finite")
        object.__setattr__(self, "history", history.copy())


@dataclass(frozen=True)
class ComplexFIRFitDiagnostics:
...
@dataclass(frozen=True)
class ComplexFIRResidualCorrection:
    """Immutable causal proper-complex FIR residual coefficient set."""
    ...
    def __post_init__(self) -> None:
        ...
        if coefficients.shape != (len(delays),):
            raise ValueError("coefficients must contain one complex value per delay")
        ...
        if not np.all(np.isfinite(coefficients)):
            raise ValueError("complex FIR coefficients must be finite")
    ...
    def metadata(self) -> dict[str, object]:
        return {
            "model_type": "causal_proper_complex_fir_residual_correction",
            "equation": "sum_d b_d * x[n-d]",
            "delays": list(self.delays),
            "tap_count": self.tap_count,
            "maximum_delay": self.maximum_delay,
            "phase_behavior": "phase-equivariant",
            "causal_padding": "zeros_before_record_or_reset_segment_start",
            "continuous_streaming": "carry ComplexFIRStreamingState",
            "physical_attribution": "not identified by this model alone",
        }

    def initial_state(self) -> ComplexFIRStreamingState:
        return ComplexFIRStreamingState(
            np.zeros(self.maximum_delay, dtype=np.complex128)
        )

    def _validate_state(self, state: ComplexFIRStreamingState) -> np.ndarray:
        if not isinstance(state, ComplexFIRStreamingState):
            raise TypeError("state must be a ComplexFIRStreamingState")
        if state.history.size != self.maximum_delay:
            raise ValueError("state history length must equal model.maximum_delay")
        return state.history

    def predict_chunk(
        self,
        signal: np.ndarray,
        state: ComplexFIRStreamingState,
    ) -> tuple[np.ndarray, ComplexFIRStreamingState]:
        ...
        return (
            ...
            ComplexFIRStreamingState(next_history),
        )
    ...
    def correct(
        ...
        """Add this correction to one independent base prediction."""
    ...
    def operation_count(
        ...
        *,
        ...
        return complex_fir_residual_correction_cost(
            self.delays,
    ...
    def save(self, path: str | Path) -> None:
        np.savez(
            ...
            model_type=np.asarray(
                "causal_proper_complex_fir_residual_correction"
            ),
    ...
    @classmethod
    def load(cls, path: str | Path) -> "ComplexFIRResidualCorrection":
        with np.load(Path(path), allow_pickle=False) as data:
            if int(data["schema_version"]) != 1:
                raise ValueError("unsupported complex FIR model schema")
            if str(data["model_type"]) != (
                "causal_proper_complex_fir_residual_correction"
            ):
                raise ValueError("unexpected complex FIR model type")
            return cls(
...
def fit_complex_fir_residual_correction(
    pa_input: np.ndarray,
    ...
    *,
...
) -> tuple[ComplexFIRResidualCorrection, ComplexFIRFitDiagnostics]:
    """Fit proper-complex FIR coefficients without normal equations."""
    ...
    if max(delay_tuple) >= length:
        raise ValueError("complex FIR memory must be shorter than each frame")
    ...
    if design.shape[1] >= design.shape[0]:
        raise ValueError("complex FIR least-squares system must be overdetermined")
    column_rms = np.sqrt(np.mean(np.abs(design) ** 2, axis=0))
    if np.any(~np.isfinite(column_rms)) or np.any(column_rms <= 0.0):
        raise ValueError("complex FIR design has invalid or all-zero columns")
    ...
    model = ComplexFIRResidualCorrection(delay_tuple, coefficients)
    ...
    if singular_values.size and singular_values[-1] > 0.0:
        ...
        condition = float("inf")
    diagnostics = ComplexFIRFitDiagnostics(
